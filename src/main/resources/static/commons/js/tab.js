 ...
 * 鐠併劏顔戠紘銈忕窗112044258閵嗭拷2994605閵嗭拷6534121閵嗭拷6271061 閸掓稑缂撻弮銉︽埂閿涳拷011/1/17
 * 
		...
		$.each(n.menus, function(j, o) {
			...
			if (o.child && o.child.length > 0) {
				...
				$.each(o.child, function(k, p) {
					menulist += '<li><div><a ref="' + p.menuid + '" href="#" rel="' + p.url + '" ><span class="icon ' + p.icon + '" >&nbsp;</span><span class="nav">' + p.menuname
							+ '</span></a></div> </li>'
				});
				menulist += '</ul>';
			}
...
function addTab(subtitle, url, icon) {
	if (!$('#tabs').tabs('exists', subtitle)) {
		...
		var currentTab = $('#tabs').tabs('getTab', subtitle); // get selected panel
...
function tabClose() {
	/*
	 * 閸欏苯鍤崗鎶芥４TAB闁銆嶉崡锟� $(".tabs-inner").dblclick(function(){ var subtitle =
	 * $(this).children(".tabs-closable").text();
	 * $('#tabs').tabs('close',subtitle); }) /*娑撴椽锟芥い鐟板幢缂佹垵鐣鹃崣鎶芥暛
	 */
...
// 瀵懓鍤穱鈩冧紖缁愭褰�title:閺嶅洭顣�msgString:閹绘劗銇氭穱鈩冧紖 msgType:娣団剝浼呯猾璇茬�
